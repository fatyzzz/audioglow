...
class OverlaySpectrumVisualizer(BaseVisualizer):
    def __init__(self, ctx, resolution, assets, config):
        ...
        self.prog = self.ctx.program(
            vertex_shader="""
                ...
                in vec2 in_uv;
                out vec2 v_uv;
                void main() { gl_Position = vec4(in_vert, 0.0, 1.0); v_uv = in_uv; }
            """,
            fragment_shader="""
                #version 330
    ...
    def update_cover(self, cover_path):
        print(f"[Spectrum] Switching to: {Path(cover_path).name}")
        ...
        if hasattr(self, "tex_palette"):
            ...
            print(f"[Spectrum] Palette applied: {self.palette[:2]}...")

    def _update_uniforms(self):
        self.prog["tex_bg"].value = 0
        self.prog["tex_fg"].value = 1
        self.prog["tex_spectrum"].value = 2
        self.prog["tex_palette"].value = 3
        self.prog["resolution"].value = tuple((self.width, self.height))
        self.prog["cover_res"].value = (
            self.config.get("cover_size", 800),
            self.config.get("cover_size", 800),
        )
        self._update_palette_texture()
